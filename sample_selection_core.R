...
# Shared sampling pipeline used by both QMD files
# =========================================================
...
group_name <- if (!exists("group_name", inherits = FALSE)) "CV" else group_name
data_excel <- if (!exists("data_excel", inherits = FALSE)) "Detroit MI.xlsx" else data_excel
fips_file  <- if (!exists("fips_file", inherits = FALSE)) "Census Tract FIPS Code Detroit MI.csv" else fips_file
n_psu <- if (!exists("n_psu", inherits = FALSE)) 40 else n_psu
...
select_one_bg_within_tract <- function(df_tract_bg) {
  if (nrow(df_tract_bg) == 1) {
    df_tract_bg %>% mutate(pik2 = 1, s2 = 1)
  } else {
    pik2 <- inclusionprobabilities(df_tract_bg$MOS, 1)
    s2 <- UPsystematic(pik2)
    df_tract_bg %>% mutate(pik2 = pik2, s2 = as.numeric(s2))
  }
}
...
tract_raw <- tract_raw %>%
  mutate(Tract = str_pad(as.character(Tract), width = 11, side = "left", pad = "0"))
...
tract_frame <- tract %>% filter(!zero_pop_flag, n_bg >= 1)
bg_frame <- bg %>% semi_join(tract_frame, by = "Tract") %>% filter(age18p > 0, TotPerson > 0)
...
bg_sample <- bg_sample %>%
  ...
  left_join(tract_sample %>% select(Tract, pik1), by = "Tract") %>%
  mutate(
